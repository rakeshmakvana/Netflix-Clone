...
const WatchPage = () => {
	...
	const fetchData = async (endpoint, setter) => {
		try {
			...
			// Handle different types of errors based on status code if needed
			if (error.response?.status === 404) {
				setter([]); // Set to empty array for trailers and similar content
			} else {
	...
	useEffect(() => {
		const fetchAllData = async () => {
		  try {
			const [trailersData, similarData, detailsData] = await Promise.all([
			  axios.get(`/api/v1/${contentType}/${id}/trailers`),
			  axios.get(`/api/v1/${contentType}/${id}/similar`),
			  axios.get(`/api/v1/${contentType}/${id}/details`),
			]);
			setTrailers(trailersData.data.trailers || []);
			setSimilarContent(similarData.data.similar || []);
			setContent(detailsData.data.content || null);
		  } catch (error) {
			console.error("Error fetching data:", error);
		  } finally {
			setLoading(false);
		...
		<div className='bg-black min-h-screen text-white'>
			<div className='mx-auto container px-4 py-8 h-full'>
				...
				<div className='flex flex-col md:flex-row items-center justify-between gap-20 max-w-6xl mx-auto'>
					...
					<img
						src={content.poster_path ? ORIGINAL_IMG_BASE_URL + content.poster_path : "fallback-image-url.jpg"}
						alt='Poster image'
